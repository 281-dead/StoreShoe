...
namespace DAO
{
    public class NHANVIENDAO
    {
        ...
        #region Tìm Kiếm Nhân Viên Theo Danh Sách
        public DataTable GetDanhSachNhanVienTimKiem(string tennv)
        {
            try
            {
                DataTable dt = new DataTable();
                string sql = "SELECT MANV as N'Mã NV',HOTEN as N'Tên NV',MALOAI as N'Mã Loại NV',SDT as N'SĐT',NGAYSINH as N'Ngày Sinh',Email as N'Email',HINHANH as N'Hình Ảnh' , CASE GIOITINH WHEN 0 THEN N'Nữ' WHEN 1 THEN N'Nam' END AS N'Giới Tính' FROM NHANVIEN WHERE DATHOIVIEC=0 AND MANV != 0 AND HOTEN LIKE N'%" + tennv + "%'";
                dt = new DataTable();
                dt = DataProvider.GetTable(sql);
                return dt;
            }
        ...
        #region Lấy danh sách Nhân Vien
        public DataTable GetDanhSachNhanVien()
        {
            try
            {
                DataTable dt = new DataTable();
                string sql = "SELECT MANV as N'Mã NV',HOTEN as N'Tên NV',MALOAI as N'Mã Loại NV',SDT as N'SĐT',NGAYSINH as N'Ngày Sinh',Email as N'Email',HINHANH as N'Hình Ảnh' , CASE GIOITINH WHEN 0 THEN N'Nữ' WHEN 1 THEN N'Nam' END AS N'Giới Tính' FROM NHANVIEN WHERE DATHOIVIEC=0";
                dt = new DataTable();
                dt = DataProvider.GetTable(sql);
                return dt;
            }
